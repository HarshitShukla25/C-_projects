...
int main() 
{
		...
		double T1=100.0,T2=100.0;
          cout << "This is a program to calculate the DLS par score for the teams in a One Day Cricket Match based on the Duckworth-Lewis System [Standard Edition] 2003."<<endl;
          cout <<"Enter the number of interruptions during the match: "<<endl;
          		cin >> number_of_interruptions;

          		 rep(i,number_of_interruptions)
          		 {   int a;
                     cout <<"In which innings did the "<<i+1<<"th interruption occur: "<<endl;
                     cin >> a;
                     if(a==1)
                     {  int overs_stop,overs_left;
                        int wickets;
                     	cout<<"Enter the overs left when the play stopped.: "<<endl;
                        cin >> overs_stop;
                        cout<<"Enter the wickets gone when the play stopped"<<endl;
                        cin >> wickets;
                        cout << "Enter the overs left when the play resumed: "<<endl;
                        cin >>  overs_left;
                        T1-=(resources[50-overs_stop][wickets]-resources[50-overs_left][wickets]);
                     ...
                     {int overs_stop,overs_left;
                        ...
                        cout << "Enter the overs left when the play resumed: "<<endl;
                        cin >>  overs_left;
                        T2-=(resources[50-overs_stop][wickets]-resources[50-overs_left][wickets]);
          		 ...
          		 cout << T1 <<" "<<T2;
          		 int G50;
          		 cout << "Enter the G50 as per the standards.(G50 stands for the average score in a 50 over ODI match."<<endl;
          		 cin >> G50;
          		 int finalscoret1;
          		 cout<<"Enter the final score of Team 1 at the end of its innings."<<endl;
          		 cin >>finalscoret1;
          		 double dlsparscore=0.0;
          		 if(T1>T2)
          		 {  double ratio=T2/T1;
          		 	cout << ratio;
          		 	dlsparscore=ratio*finalscoret1;

          		 }
          		 else if(T1==T2)
          		 {
          		 	dlsparscore=finalscoret1;
          		 }
          		 else
          		 {
          		 	dlsparscore=finalscoret1+(G50*(T2-T1)/100.0);
          		 }
          		 cout << "The DLS par score is: "<<floor(dlsparscore);
